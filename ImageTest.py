...
def extra_plate():
    ...
    if plate_contour is not None:
        # Vẽ đường viền xung quanh biển số vào ảnh gray, -1 cho biết vẽ tất cả các đường viền, 2 là độ dày
        cv2.drawContours(image, plate_contour, -1, (0, 255, 0), 1)
        ...
        if len(x) > 0 and len(y) > 0:
            ...
            reCombinedCrop = reCombined[topx:bottomx+3, topy+3:bottomy-3]
...
def update_image():
    ...
    (h, w) = img_cv2.shape[:2]
    if 2*h < w:
        img_cv2 = cv2.resize(img_cv2, (500, (int(h*(500/w)))),
                             interpolation=cv2.INTER_LINEAR)
    else:
        img_cv2 = cv2.resize(img_cv2, (500, 300),
                             interpolation=cv2.INTER_LINEAR)
